...
//  TripStori
//
//  Created by Jesse Tello Jr. on 10/14/16.
//  Copyright © 2016 Tello. All rights reserved.
//

import UIKit

public class TSSpinner: UIView {

    public class var sharedInstance: TSSpinner {
        struct Singleton {
            static let instance = TSSpinner(frame: CGRect.zero)
        }
    ...
    //
    @discardableResult
    public class func show(_ title: String, animated: Bool = true) -> TSSpinner {
        
        let spinner = TSSpinner.sharedInstance
        ...
        if spinner.superview == nil {
            ...
            UIView.animate(withDuration: 0.33, delay: 0.0, options: .curveEaseOut, animations: {
                ...
                NotificationCenter.default.addObserver(
                    spinner,
                    selector: #selector(TSSpinner.updateFrame),
    ...
    //
    @discardableResult
    public class func show(duration: Double, title: String, animated: Bool = true) -> TSSpinner {
        let spinner = TSSpinner.show(title, animated: animated)
    ...
    public class func show(delay: Double, title: String, animated: Bool = true) {
        ...
        TSSpinner.sharedInstance.delay(delay, completion: {
            if let index = delayedTokens.index(of: token) {
                delayedTokens.remove(at: index)
                _ = TSSpinner.show(title, animated: animated)
            }
        })
    }
    ...
    ///
    @discardableResult
    public class func show(progress: Double, title: String) -> TSSpinner {
        let spinner = TSSpinner.show(title, animated: false)
    ...
    let frameSize = CGSize(width: 125.0, height: 125.0)
    ...
    public func updateFrame() {
        if let containerView = TSSpinner.containerView() {
            TSSpinner.sharedInstance.frame = containerView.bounds
        }
    }
